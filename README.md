...
- **专业动作库**：302 个动作（workout-guide 全量，含 906 帧图解），覆盖胸 / 背 / 腿 / 臀腿 / 肩 / 手臂 / 核心 / 有氧 / 拉伸 9 大类，含中英文术语、器械图示、男女差异化推荐重量区间
- **免登录动作库浏览页**：`pages/library` 按部位展示全部动作，`anim` 分包内 `pages/exercise-detail?ex=id` 展示动作图解与三帧演示，供微信搜一搜收录
- **训练记录**：自由训练 / 模板训练双模式；8 套男女差异化模板（全身分化男/女版、PPL、上下肢分化、铁馆器械男/女版、进阶计划），训练中可随时换模板
...
miniprogram/          小程序前端（9 页面 + 工具层 + 数据种子）
  ...
  data/               动作库（302 个）与模板（8 套）前端兜底种子
  anim/               分包：906 帧动作图解 + 动作详情演示页
